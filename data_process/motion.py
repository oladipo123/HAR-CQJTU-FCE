...
DATASET_PATH = r'G:\Human activity\data\motion'
...
def preprocess(path, path_save, version, target_window=50, seq_len=20):
    data_acc, label_acc = load_sensor_data(os.path.join(path, 'Accelerometer\B_Accelerometer_data'), seq_len, target_window)
    data_gyro, label_gyro = load_sensor_data(os.path.join(path, 'Gyroscope\C_Gyroscope_data'), seq_len, target_window)
    data = []
    label = []
