...
class Preprocess4Mask:
    ...
    def __call__(self, instance):
        ...
        # print(f"the instance shape is{shape}")

        # the number of prediction is sometimes less than max_pred when sequence is short
        # print(f"the mask_ratio is {self.mask_ratio}")
...
class LIBERTDataset4Pretrain(Dataset):
    """ Load sentence pair (sequential or random order) from corpus """
    def __init__(self, data, pipeline=[], pipeline1=[]):
        super().__init__()
        self.pipeline = pipeline
        self.data = data
    ...
    def __getitem__(self, index):
        instance = self.data[index]
        ori_instance = instance.copy()
        for proc in self.pipeline1:
            ori_instance = proc(ori_instance)
        ori_instance = np.array(ori_instance)
        ori_seq = ori_instance
        ori_instance = ori_instance[:, :6]
        # print(instance.shape)
        # print(f"the index is{index}")
        for proc in self.pipeline:
            instance = proc(instance)
        mask_seq, masked_pos, seq = instance
        # plot_reconstruct_sensor(torch.from_numpy(ori_instance), torch.from_numpy(mask_seq))
        # print(f"mask pos is{masked_pos}")
        return torch.from_numpy(mask_seq), torch.from_numpy(masked_pos).long(), torch.from_numpy(seq), torch.from_numpy(ori_seq)
...
def handle_argv(target, config_train, prefix):
    ...
    parser.add_argument('dataset', type=str, help='Dataset name', choices=['hhar', 'motion', 'uci', 'shoaib', 'myowner'])
...
def handle_argv_simple():
    ...
    parser.add_argument('dataset', type=str, help='Dataset name', choices=['hhar', 'motion', 'uci', 'shoaib', 'merge', 'myowner'])
    parser.add_argument('dataset_version',  type=str, help='Dataset version', choices=['10_100', '20_120'])
    args = parser.parse_args()
    dataset_cfg = load_dataset_stats(args.dataset, args.dataset_version)
    if dataset_cfg is None:
...
def load_pretrain_data_config(args):
    ...
    print(f"path is {args.data_path}")
    data = np.load(args.data_path).astype(np.float32)
    labels = np.load(args.label_path).astype(np.float32)
