...
class Trainer(object):
    ...
    def pretrain(self, func_loss, func_forward, func_evaluate
        ...
        for e in range(self.cfg.n_epochs):
            ...
            for i, batch in enumerate(data_loader_train):
                ...
                if i == 65:
                    label = 1
                loss = func_loss(model, batch, label)
    ...
    def run(self, func_forward, func_evaluate, data_loader, model_file=None, data_parallel=False, load_self=False):
        """ Evaluation Loop """
        print(f"the model_file is {model_file}")
        ...
        for e in range(self.cfg.n_epochs):
            ...
            for i, batch in enumerate(data_loader_train):
                ...
                if self.cfg.total_steps and self.cfg.total_steps < global_step:
                    print('The Total Steps have been reached.')
                    return
            train_acc, train_f1, train_kappa = self.run(func_forward, func_evaluate, data_loader_train)
            test_acc, test_f1, test_kappa = self.run(func_forward, func_evaluate, data_loader_test)
            vali_acc, vali_f1, vali_kappa = self.run(func_forward, func_evaluate, data_loader_vali)
            print('Epoch %d/%d : Average Loss %5.4f, Accuracy: %0.3f/%0.3f/%0.3f, F1: %0.3f/%0.3f/%0.3f, Cohen Kappa: %0.3f/%0.3f/%0.3f'
                  % (e+1, self.cfg.n_epochs, loss_sum / len(data_loader_train), train_acc, vali_acc, test_acc, train_f1, vali_f1, test_f1, train_kappa, vali_kappa, test_kappa))
            # print("Train execution time: %.5f seconds" % (time_sum / len(self.data_loader)))
            if vali_acc > vali_acc_best:
                vali_acc_best = vali_acc
                best_stat = (train_acc, vali_acc, test_acc, train_f1, vali_f1, test_f1, train_kappa, vali_kappa, test_kappa)
                model_best = copy.deepcopy(model.state_dict())
                self.save(0)
        self.model.load_state_dict(model_best)
        print('The Total Epoch have been reached.')
        print('Best Accuracy: %0.3f/%0.3f/%0.3f, F1: %0.3f/%0.3f/%0.3f, kappa: %0.3f/%0.3f/%0.3f' % best_stat)
