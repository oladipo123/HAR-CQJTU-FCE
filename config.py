...
def create_io_config(args, dataset_name, version, pretrain_model=None, target='pretrain'):
    # data_path = os.path.join('dataset', dataset_name, 'augment', 'data_' + version + '.npy')
    # label_path = os.path.join('dataset', dataset_name, 'augment', 'label_' + version + '.npy')
    ...
    print(f"the label_path is {label_path}")
    args.data_path = data_path
    args.label_path = label_path

    # save_path = os.path.join('saved', target + "_" + dataset_name + "_augment_" + version)  # + "_temp"
    # save_path = os.path.join('saved', target + "_" + dataset_name + "_" + version)  # + "_temp"
    # save_path = os.path.join('saved', target + "_" + dataset_name + "_attention_" + version)  # + "_temp"
    # save_path = os.path.join('saved', target + "_" + dataset_name + "_TXMixer_leaky_gelu_standout_" + version)  # + "_temp"
    # save_path = os.path.join('saved', target + "_" + dataset_name + "_TimeCMA_0.2_mask_" + version)  # + "_temp"
    save_path = os.path.join('saved', target + "_" + dataset_name + "_TXMixer_leaky_gelu_standout_2times_0.2_mask_" + version)  # + "_temp"
    # save_path = os.path.join('saved', target + "_" + dataset_name + "_TXMixer_leaky_gelu_standout_2times_exchange_" + version)  # + "_temp"
    # save_path = os.path.join('saved', target + "_" + dataset_name + "_TXMixer_leaky_relu_0.1_" + version)  # + "_temp"
...
def load_dataset_label_names(dataset_config, label_index):
    for p in dir(dataset_config):
        # print(getattr(dataset_config, p))
