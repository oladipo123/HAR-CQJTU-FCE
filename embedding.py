...
def generate_embedding_or_output(args, save=False, output_embed=True):
    ...
    def func_forward(model, batch):
        seqs, label = batch
        print(f"seqs shape is {seqs.shape}")
        print(f"label shape is {label.shape}")
        embed = model(seqs)
        flops, params = profile(model, inputs=(seqs,), verbose=False)
        print(f"FLOPs: {flops}, Parameters: {params}")
        # print(f"embed shape is {embed.shape}")
    ...
    # trainer.save()
    if save:
        # save_name = 'embed_Mixer_leaky_gelu_layer_standout_' + args.model_file.split('.')[0] + '_' + args.dataset + '_' + args.dataset_version
        save_name = 'embed_Mixer_leaky_gelu_layer_standout_2times_0.2_mask_' + args.model_file.split('.')[0] + '_' + args.dataset + '_' + args.dataset_version
        # save_name = 'embed_TimeCMA_' + args.model_file.split('.')[0] + '_' + args.dataset + '_' + args.dataset_version
        # save_name = 'embed_Mixer_leaky_gelu_layer_standout_2times_exchange_' + args.model_file.split('.')[0] + '_' + args.dataset + '_' + args.dataset_version
        # save_name = 'embed_Mixer_leaky_gelu_layer_standout_' + args.model_file.split('.')[0] + '_' + args.dataset + '_' + args.dataset_version
        # save_name = 'embed_Mixer_leaky_relu_layer_0.1_' + args.model_file.split('.')[0] + '_' + args.dataset + '_' + args.dataset_version
...
def load_embedding_label(model_file, dataset, dataset_version):
    embed_name = 'embed_Mixer_leaky_gelu_layer_standout_2times_0.2_mask_' + model_file + '_' + dataset + '_' + dataset_version
    # embed_name = 'embed_TimeCMA_' + model_file + '_' + dataset + '_' + dataset_version
    # embed_name = 'embed_Mixer_leaky_gelu_layer_standout_2times_exchange_' + model_file + '_' + dataset + '_' + dataset_version
    # embed_name = 'embed_Mixer_leaky_gelu_layer_standout_' + model_file + '_' + dataset + '_' + dataset_version
    # embed_name = 'embed_Mixer_leaky_gelu_layer_standout_' + model_file + '_' + dataset + '_' + dataset_version
    # embed_name = 'embed_Mixer_leaky_relu_layer_0.1_' + model_file + '_' + dataset + '_' + dataset_version
    label_name = 'label_' + dataset_version
    print(f"the embed name is {embed_name}")
...
if __name__ == "__main__":
    ...
    print(f"label_names is {label_names}")
